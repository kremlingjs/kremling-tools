...
const postcssKremling = require("postcss-kremling");
...
module.exports = function (source) {
  ...
  postcss([
    ...pluginsInit,
    postcssKremling({ id: `p${id}`, namespace: kremlingNamespace }),
  ])
    .process(source, {
      ...restOfOptions,
      to: "./",
    ...
    .then((result) => {
      if (result.css) {
        callback(
          null,
          `module.exports = { styles: '${result
            .toString()
            .replace(/(\s)+/g, " ")
            .replace(
              /'/g,
              `\\'`
            )}', id: 'p${id}', ${kremlingNamespaceString} };`
        );
      } else {
        callback(
          null,
          `module.exports = { styles: '', id: 'p${id}', ${kremlingNamespaceString} };`
        );
      }
    });
};
